...
function toggleTmFeed() {
var feed = document.getElementById('tmFeed');
var btn = document.querySelector('.tm2-rp-toggle');
var tab = document.getElementById('tmFeedTab');
if (!feed) return;
var c = feed.classList.toggle('collapsed');
if (btn) btn.textContent = c ? '▶' : '◀';
if (tab) tab.classList.toggle('visible', c);
}
setInterval(function() {
...
function mkSvgIcon(sev) {
var cfg = {
'Critical': { cls:'mkr-crit', sz:30, html:'<span class="mkr-ring"></span><span class="mkr-ring mkr-ring-b"></span><span class="mkr-glow"></span><span class="mkr-core"></span>' },
'High':     { cls:'mkr-high', sz:22, html:'<span class="mkr-ring"></span><span class="mkr-glow"></span><span class="mkr-core"></span>' },
'Medium':   { cls:'mkr-med',  sz:16, html:'<span class="mkr-glow"></span><span class="mkr-core"></span>' },
'Low':      { cls:'mkr-low',  sz:10, html:'<span class="mkr-core"></span>' }
}[sev] || { cls:'mkr-low', sz:10, html:'<span class="mkr-core"></span>' };
return L.divIcon({
className: '',
html: '<div class="mkr ' + cfg.cls + '">' + cfg.html + '</div>',
iconSize:   [cfg.sz, cfg.sz],
iconAnchor: [cfg.sz / 2, cfg.sz / 2],
popupAnchor:[0, -(cfg.sz / 2 + 5)]
});
}
function mkPopup(r, col) {
var sevLabel = SEV_DISPLAY[r.sev] ? (SEV_DISPLAY[r.sev][LANG] || r.sev) : r.sev;
var typeStr = esc(rt(r.type));
var srcBadge = r.userSubmitted
? '<span style="font-size:9px;font-weight:700;letter-spacing:1px;color:#00ff88;background:rgba(0,255,136,0.1);border:1px solid rgba(0,255,136,0.25);padding:2px 7px;border-radius:3px;font-family:monospace;">▶ LIVE</span>'
: '<span style="font-size:9px;font-weight:700;letter-spacing:1px;color:#6b7280;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);padding:2px 7px;border-radius:3px;font-family:monospace;">SIM</span>';
return '<div style="font-family:\'Inter\',sans-serif;min-width:200px;max-width:260px;">' +
'<div style="height:2px;background:linear-gradient(90deg,' + col + ',transparent);margin:-1px -1px 10px;border-radius:2px 2px 0 0;"></div>' +
'<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">' +
'<span style="font-size:9px;font-weight:800;letter-spacing:1.5px;font-family:\'JetBrains Mono\',monospace;color:' + col + ';background:' + col + '18;border:1px solid ' + col + '40;padding:2px 8px;border-radius:3px;">' + sevLabel + '</span>' +
'<span style="font-size:9px;font-weight:600;letter-spacing:0.5px;color:rgba(0,212,255,0.8);background:rgba(0,212,255,0.08);border:1px solid rgba(0,212,255,0.2);padding:2px 8px;border-radius:3px;">' + typeStr + '</span>' +
srcBadge +
'</div>' +
'<div style="font-size:12px;font-weight:700;color:#fff;line-height:1.45;margin-bottom:6px;">' + esc(rt(r.title)) + '</div>' +
'<div style="font-size:11px;color:#8899aa;line-height:1.6;margin-bottom:8px;">' + esc(rt(r.desc)) + '</div>' +
'<div style="display:flex;align-items:center;font-size:10px;color:#55667a;font-family:monospace;">📍 ' + esc(r.area) + '</div>' +
'</div>';
}
var targetLayer;
...
var icon = mkSvgIcon(r.sev);
var marker = L.marker([city.lat + jLat, city.lng + jLng], { icon: icon });
marker.bindPopup(mkPopup(r, col), { className:'dark-popup', maxWidth:280 });
...
b.classList.remove('mcb-active','tm-ctrl-active','tm2-vbtn-on');
});
btn.classList.add('mcb-active');
btn.classList.add('tm2-vbtn-on');
renderMarkers();
}
...
timelineDays = days;
document.querySelectorAll('.mcb-tl-btn,.tm-tl-btn,.tm2-tl-btn').forEach(function(b) {
b.classList.remove('mcb-tl-active','tm-tl-active','tm2-tl-on');
});
btn.classList.add('tm2-tl-on');
renderMarkers();
renderList();
renderSidebarStats();
}
...
activeFilter = f;
document.querySelectorAll('.tm2-sev-btn,.tm-sev-btn,.fbtn').forEach(function(b) {
b.classList.remove('on','tm-sev-on','tm2-sev-on');
});
btn.classList.add('on');
btn.classList.add('tm2-sev-on');
renderList();
renderMarkers();
}
...
welcome.innerHTML =
'<div class="cw-hero">' +
'<div class="cw-shield">🛡️</div>' +
'<div class="cw-title">' + t('chat-welcome') + '</div>' +
'<div class="cw-sub">' + (isAr ? 'ذكاء اصطناعي متخصص في كشف الاحتيال الكويتي' : 'AI specialized in detecting Kuwait-targeted scams') + '</div>' +
'</div>' +
'<div class="cw-features">' +
'<div class="cw-feat"><span class="cwf-ico">📱</span><span class="cwf-txt">' + (isAr ? 'رسائل SMS مشبوهة' : 'Suspicious SMS messages') + '</span></div>' +
'<div class="cw-feat"><span class="cwf-ico">🔗</span><span class="cwf-txt">' + (isAr ? 'روابط تصيد' : 'Phishing links & URLs') + '</span></div>' +
'<div class="cw-feat"><span class="cwf-ico">📧</span><span class="cwf-txt">' + (isAr ? 'بريد إلكتروني احتيالي' : 'Scam emails & offers') + '</span></div>' +
'<div class="cw-feat"><span class="cwf-ico">📞</span><span class="cwf-txt">' + (isAr ? 'مكالمات وهمية' : 'Phone call scams') + '</span></div>' +
'</div>' +
'<div class="cw-ready"><span class="cw-dot"></span>' + (isAr ? 'الصق رسالة مشبوهة أو اختر مثال بالأسفل' : 'Paste a suspicious message or tap an example below') + '</div>';
...
var verdictCard = '';
var verdictMatch = html.match(/^[^\n]*?(🚨\s*SCAM\s*DETECTED|⚠️\s*SUSPICIOUS|✅\s*LIKELY\s*LEGITIMATE|🚨\s*تم\s*الكشف|⚠️\s*مشبوه|✅\s*آمن)[^\n]*/i);
if (verdictMatch) {
var line = verdictMatch[0];
var vtype = 'sus';
if (/🚨|SCAM|تم\s*الكشف/i.test(line)) vtype = 'scam';
else if (/✅|LEGITIMATE|آمن/i.test(line)) vtype = 'safe';
var vLabel = { scam: 'SCAM DETECTED', sus: 'SUSPICIOUS', safe: 'LIKELY LEGITIMATE' }[vtype];
var vIco = { scam: '🚨', sus: '⚠️', safe: '✅' }[vtype];
var vConf = { scam: 'HIGH THREAT', sus: 'MODERATE RISK', safe: 'LOW RISK' }[vtype];
verdictCard = '<div class="vrd vrd-' + vtype + '">' +
'<div class="vrd-top"><span class="vrd-ico">' + vIco + '</span><span class="vrd-label">' + vLabel + '</span></div>' +
'<div class="vrd-bar"><div class="vrd-bar-fill vrd-fill-' + vtype + '"></div></div>' +
'<div class="vrd-conf">' + vConf + '</div>' +
'</div>';
html = html.replace(verdictMatch[0], '').replace(/^\s*\n/, '');
}
...
if (pwdLower === cl) return true;
if (cl.length >= 6 && pwdLower.indexOf(cl) >= 0) return true;
return false;
});
var checks = [{
key: 'pwd-c1',
...
return stripped.length >= 4 && w.length >= 4 && stripped.indexOf(w) !== -1;
});
if (hasDictWord)
...
var fd = new FormData();
fd.append('url', url);
var resp = await fetch('https://urlhaus-api.abuse.ch/v1/url/', {
method: 'POST',
body: fd
});
...
urlhausRef: data.urlhaus_reference || ''
};
} catch (e) {
return null;
}
...
try {
var resp = await fetch('https://rdap.org/domain/' + encodeURIComponent(domain), {
...
var reg = events.find(function(e) { return e.eventAction === 'registration'; });
var upd = events.find(function(e) { return e.eventAction === 'last changed'; });
if (reg) {
var regDate = new Date(reg.eventDate);
result.created = regDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
var ageDays = Math.floor((Date.now() - regDate.getTime()) / 86400000);
result.ageDays = ageDays;
result.ageHuman = ageDays < 30 ? ageDays + 'd' : ageDays < 365 ? Math.floor(ageDays / 30) + ' months' : (ageDays / 365).toFixed(1) + ' years';
result.isNew = ageDays < 90;
}
if (upd) {
result.updated = new Date(upd.eventDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
}
var entities = data.entities || [];
var registrar = entities.find(function(e) { return e.roles && e.roles.indexOf('registrar') >= 0; });
if (registrar) {
var vc = registrar.vcardArray && registrar.vcardArray[1];
if (vc) {
var fn = vc.find(function(v) { return v[0] === 'fn'; });
if (fn) result.registrar = fn[3];
}
if (!result.registrar && registrar.handle) result.registrar = registrar.handle;
}
result.status = (data.status || []).slice(0, 3).join(' · ');
return result;
} catch (e) {
return null;
}
}
var _cveKeyword = 'Windows';
var _cveLoaded = false;
var CVE_SEV_EXPLAIN = {
CRITICAL: 'Extremely dangerous — an attacker can fully take over your device with zero effort from you. Patch immediately.',
HIGH:     'Serious risk — attacker could steal your data, install malware, or control your device. Update as soon as possible.',
MEDIUM:   'Moderate risk — requires some interaction to exploit (e.g. opening a file or clicking a link). Update soon.',
LOW:      'Minor risk — difficult to exploit and limited impact. Patch during your next regular update cycle.'
};
function cveImpactType(desc) {
var d = desc.toLowerCase();
if (/remote code exec|arbitrary code|execute.*command|rce/i.test(d))   return { label: 'Remote Code Execution', icon: '💥', cls: 'cve-impact-rce' };
if (/privilege escalat|elevat.*privilege|gain.*root|local.*priv/i.test(d)) return { label: 'Privilege Escalation', icon: '⬆️', cls: 'cve-impact-pe' };
if (/information disclosure|sensitive.*data|leak.*data|expose.*data|obtain.*credential/i.test(d)) return { label: 'Data Exposure', icon: '🔓', cls: 'cve-impact-info' };
if (/denial.of.service|\bdos\b|crash|unavailability|infinite loop/i.test(d)) return { label: 'Denial of Service', icon: '⛔', cls: 'cve-impact-dos' };
if (/authentication bypass|bypass.*auth|unauthenticat/i.test(d)) return { label: 'Auth Bypass', icon: '🔑', cls: 'cve-impact-auth' };
if (/cross.site.script|\bxss\b/i.test(d)) return { label: 'Cross-Site Scripting', icon: '🌐', cls: 'cve-impact-xss' };
if (/sql.inject/i.test(d)) return { label: 'SQL Injection', icon: '💉', cls: 'cve-impact-sqli' };
if (/buffer overflow|heap.*corrupt|use.after.free|memory corrupt|out.of.bounds/i.test(d)) return { label: 'Memory Corruption', icon: '⚠️', cls: 'cve-impact-mem' };
if (/command inject|os command/i.test(d)) return { label: 'Command Injection', icon: '⚡', cls: 'cve-impact-rce' };
if (/path traversal|directory traversal/i.test(d)) return { label: 'Path Traversal', icon: '📂', cls: 'cve-impact-info' };
return { label: 'Security Vulnerability', icon: '🛡', cls: 'cve-impact-gen' };
}
var CVE_ACTION = {
Windows:          'Open Start → Settings → Windows Update → Check for updates, then restart.',
Android:          'Go to Settings → System → Software Update and install all pending updates.',
Chrome:           'In Chrome, click Menu (⋮) → Help → About Google Chrome — it updates automatically.',
WhatsApp:         'Open your App Store or Google Play, search WhatsApp and tap Update.',
iOS:              'Go to Settings → General → Software Update and install the latest iOS.',
'Microsoft Office': 'Open any Office app → File → Account → Update Options → Update Now.',
Cisco:            'Contact your network administrator to apply Cisco security advisories immediately.',
banking:          'Open your bank\'s app store page and update to the latest version. Avoid using the app until patched.',
VPN:              'Update your VPN client from the vendor\'s official website or your IT department.',
Telegram:         'Update Telegram from Google Play or the App Store to the latest version.'
};
function loadCVEPage() {
if (!_cveLoaded) {
_cveLoaded = true;
fetchCVEs();
}
}
function setCVEFilter(kw, btn) {
_cveKeyword = kw;
document.querySelectorAll('.cve-chip').forEach(function(b) { b.classList.remove('active'); });
if (btn) btn.classList.add('active');
fetchCVEs();
}
async function fetchCVEs() {
var results = document.getElementById('cveResults');
var statsBar = document.getElementById('cveStatsBar');
if (!results) return;
results.innerHTML = '<div class="cve-loading"><div class="cve-load-spinner"></div><div class="cve-load-text">Querying NVD for <strong>' + _cveKeyword + '</strong> vulnerabilities…</div></div>';
if (statsBar) statsBar.style.display = 'none';
var now = new Date();
var past = new Date(now.getTime() - 90 * 86400000);
var fmt = function(d) { return d.toISOString().replace(/\.\d{3}Z$/, '.000'); };
var apiUrl = 'https://services.nvd.nist.gov/rest/json/cves/2.0?keywordSearch=' +
encodeURIComponent(_cveKeyword) +
'&resultsPerPage=20&noRejected&pubStartDate=' + fmt(past) + '&pubEndDate=' + fmt(now);
try {
...
results.innerHTML =
'<div class="cve-error">' +
'<div class="cve-error-ico">⚡</div>' +
'<div class="cve-error-msg">NVD is rate-limited to 8 requests per 30 seconds. Wait a moment and try again.<br><small style="opacity:0.5">' + (e.message || '') + '</small></div>' +
'<button class="cve-refresh-btn" onclick="fetchCVEs()" style="margin-top:12px;padding:8px 20px">↻ Retry</button>' +
'</div>';
}
var lu = document.getElementById('cveLastUpdate');
...
var statsBar = document.getElementById('cveStatsBar');
if (!results) return;
var vulns = (data.vulnerabilities || []).filter(function(v) {
var descs = (v.cve.descriptions || []);
var en = descs.find(function(d) { return d.lang === 'en'; });
return en && en.value && en.value.length > 20 && !/^\*\* (RESERVED|REJECT)/.test(en.value);
});
if (vulns.length === 0) {
results.innerHTML =
'<div class="cve-empty">' +
'<div style="font-size:2rem;margin-bottom:12px">🔍</div>' +
'<div style="font-size:0.9rem;color:var(--text);margin-bottom:8px">No confirmed CVEs for <strong>' + _cveKeyword + '</strong> in the last 90 days</div>' +
'<div style="font-size:0.72rem">This is good news — try another product or check back later.</div>' +
'</div>';
return;
}
var counts = { CRITICAL: 0, HIGH: 0, MEDIUM: 0, LOW: 0, UNKNOWN: 0 };
var action = CVE_ACTION[_cveKeyword] || 'Apply the latest security updates from the vendor as soon as possible.';
var cards = vulns.map(function(v) {
var cve = v.cve;
var id = cve.id || '—';
var descArr = cve.descriptions || [];
var descObj = descArr.find(function(d) { return d.lang === 'en'; }) || descArr[0];
var fullDesc = descObj ? descObj.value : '';
var shortDesc = fullDesc.length > 280 ? fullDesc.slice(0, 280) + '…' : fullDesc;
var metrics = cve.metrics || {};
var severity = 'UNKNOWN', score = null, cvssVer = '', vectorStr = '';
...
if (m31)      { severity = (m31.cvssData.baseSeverity || 'UNKNOWN').toUpperCase(); score = m31.cvssData.baseScore; cvssVer = '3.1'; vectorStr = m31.cvssData.vectorString || ''; }
else if (m30) { severity = (m30.cvssData.baseSeverity || 'UNKNOWN').toUpperCase(); score = m30.cvssData.baseScore; cvssVer = '3.0'; vectorStr = m30.cvssData.vectorString || ''; }
else if (m2)  { severity = (m2.baseSeverity || 'UNKNOWN').toUpperCase(); score = m2.cvssData.baseScore; cvssVer = '2.0'; vectorStr = m2.cvssData.vectorString || ''; }
var sevKey = ['CRITICAL','HIGH','MEDIUM','LOW'].indexOf(severity) >= 0 ? severity : 'UNKNOWN';
counts[sevKey] = (counts[sevKey] || 0) + 1;
var sevCls = { CRITICAL:'cve-sev-crit', HIGH:'cve-sev-high', MEDIUM:'cve-sev-med', LOW:'cve-sev-low', UNKNOWN:'cve-sev-unknown' }[sevKey] || 'cve-sev-unknown';
var pubDate = cve.published ? new Date(cve.published).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
var impact = cveImpactType(fullDesc);
var isKEV = !!cve.cisaExploitAdd;
var cwes = (cve.weaknesses || []).map(function(w) {
return (w.description || []).map(function(d) { return d.value; }).filter(Boolean).join(', ');
}).filter(Boolean).slice(0, 2).join(' · ');
var attackVec = '';
if (vectorStr) {
var avMatch = vectorStr.match(/AV:([NALP])/);
if (avMatch) attackVec = { N: 'Network', A: 'Adjacent', L: 'Local', P: 'Physical' }[avMatch[1]] || '';
}
var topRef = (cve.references || []).find(function(r) { return r.url && /github|microsoft|google|apple|cisco|nvd|kb\.|\/(advisory|security|bulletin)/i.test(r.url); }) || (cve.references && cve.references[0]);
return '<div class="cve-card ' + sevCls + '">' +
'<div class="cve-card-head">' +
'<div class="cve-head-left">' +
'<span class="cve-id">' + id + '</span>' +
'<span class="cve-sev-badge ' + sevCls + '">' + severity + '</span>' +
(score !== null ? '<span class="cve-score-pill">' + score + '<span class="cve-score-ver">CVSS' + cvssVer + '</span></span>' : '') +
(isKEV ? '<span class="cve-kev-badge">⚡ ACTIVELY EXPLOITED</span>' : '') +
'</div>' +
'<span class="cve-date">' + pubDate + '</span>' +
'</div>' +
'<div class="cve-meta-row">' +
'<span class="cve-impact-tag ' + impact.cls + '">' + impact.icon + ' ' + impact.label + '</span>' +
(attackVec ? '<span class="cve-vector-tag">🌐 ' + attackVec + ' Attack</span>' : '') +
(cwes ? '<span class="cve-cwe-tag">' + cwes + '</span>' : '') +
'</div>' +
'<div class="cve-desc-block">' +
'<div class="cve-desc-lbl">WHAT HAPPENED</div>' +
'<div class="cve-desc-text">' + shortDesc + '</div>' +
'</div>' +
'<div class="cve-explain-block ' + sevCls + '-explain">' +
'<span class="cve-explain-ico">' + (sevKey === 'CRITICAL' ? '🚨' : sevKey === 'HIGH' ? '⚠️' : sevKey === 'MEDIUM' ? '⚡' : 'ℹ️') + '</span>' +
'<span class="cve-explain-text">' + (CVE_SEV_EXPLAIN[sevKey] || '') + '</span>' +
'</div>' +
'<div class="cve-action-block">' +
'<div class="cve-action-lbl">✅ WHAT YOU SHOULD DO</div>' +
'<div class="cve-action-text">' + action + '</div>' +
'</div>' +
'<div class="cve-card-footer">' +
'<a class="cve-nvd-link" href="https://nvd.nist.gov/vuln/detail/' + id + '" target="_blank" rel="noopener noreferrer">View full details on NVD ↗</a>' +
(topRef ? '<a class="cve-ref-pill" href="' + topRef.url + '" target="_blank" rel="noopener noreferrer">Vendor advisory ↗</a>' : '') +
'</div>' +
'</div>';
}).join('');
...
document.getElementById('cveSHigh').textContent = counts.HIGH;
document.getElementById('cveSMed').textContent  = counts.MEDIUM;
document.getElementById('cveSLow').textContent  = counts.LOW;
document.getElementById('cveSTotal').textContent = vulns.length;
}
}
...
var rdap    = di[0];
var urlhaus = di[1];
var rdapEl = document.getElementById('ldsRDAP');
if (rdapEl) {
...
if (!sp || sp.classList.contains('sp-out')) return;
sp.classList.add('sp-out');
setTimeout(function() {
sp.classList.add('sp-gone');
checkFirstVisit();
}, 700);
}
function initWithSplash() {
init();
var sp = document.getElementById('splash');
if (!sp) {
checkFirstVisit();
...
document.addEventListener('DOMContentLoaded', initWithSplash);
} else {
initWithSplash();
}
var _newsFilter = 'all';
...
var feeds = [
{ url: 'https://thehackernews.com/feeds/posts/default', label: 'The Hacker News' },
{ url: 'https://krebsonsecurity.com/feed/', label: 'Krebs on Security' },
{ url: 'https://www.bleepingcomputer.com/feed/', label: 'Bleeping Computer' },
{ url: 'https://feeds.feedburner.com/securityweek', label: 'Security Week' }
];
var terms = ['phishing','scam','fraud','otp','bank','whatsapp','malware','ransomware','identity','crypto','hack','breach','social engineering','cyber','attack'];
var fetched = [];
var done = 0;
function finalize() {
fetched.sort(function(a, b) { return (b.rawDate || 0) - (a.rawDate || 0); });
if (fetched.length > 0) {
...
fetch('https://api.rss2json.com/v1/api.json?rss_url=' + encodeURIComponent(feed.url) + '&count=20')
.then(function(r) { return r.json(); })
.then(function(data) {
if (data.status === 'ok' && data.items) {
data.items.forEach(function(item) {
var link = (item.link || item.guid || '').trim();
if (!link || link.length < 20) return;
try {
var parsed = new URL(link);
if (parsed.pathname === '/' || parsed.pathname === '') return;
} catch(e) { return; }
var text = (item.title + ' ' + (item.description || '')).toLowerCase();
var score = terms.reduce(function(s, t) { return s + (text.indexOf(t) >= 0 ? 1 : 0); }, 0);
if (score < 1) return;
var desc = (item.description || '').replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
desc = desc.length > 200 ? desc.slice(0, 200) + '…' : desc;
if (!desc) desc = item.title;
var cat = (text.indexOf('bank') >= 0 || text.indexOf('otp') >= 0 || text.indexOf('finance') >= 0) ? 'banking' :
(text.indexOf('invest') >= 0 || text.indexOf('crypto') >= 0) ? 'investment' : 'phishing';
var pub = item.pubDate ? new Date(item.pubDate) : new Date();
fetched.push({
id: 'live-' + fetched.length,
source: feed.label,
category: cat,
categoryLabel: cat === 'banking' ? 'Banking Fraud' : cat === 'investment' ? 'Investment Scam' : 'Cyber Threat',
title: item.title,
desc: desc,
